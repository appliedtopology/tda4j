...
private[barcode] object HopcroftKarp:
  ...
  def maximumMatching(nLeft: Int, nRight: Int, adjacency: Int => Seq[Int]): Array[Int] =
    ...
    while bfs() do for u <- 0 until nLeft do if matchLeft(u) == -1 then dfs(u)
...
private[barcode] object Hungarian:
  ...
  /** `cost` must be square (`n x n`), all entries finite. Returns `(assignment, totalCost)` where `assignment(j)` is
    ...
    *
    * Classic 1-indexed potential formulation (row/column `0` are sentinels): `u`/`v` are the row/column potentials,
    * `p(j)` is the row currently assigned to column `j` (`0` = none yet), `way(j)` records the augmenting path taken to
    * reach column `j` so the final augmentation can be replayed.
    */
