...
/** Class representing an abstract simplex. Abstract simplices are given by sets (of totally ordered vertices) and
  * inherit from `Cell` so that the class has a `boundary` and a `dim` method.
  *
