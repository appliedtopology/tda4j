...
/** `cubeOrdering`/`Cube_is_OrderedCell` -- split into their own file from `Cubical.scala` (where the opaque type
  * `Cube` and its companion object live) rather than kept alongside them, and this split is required for
  * correctness, not just organizational preference. Same underlying reason as `SimplexOrderedCell.scala`'s split
  * from `Simplex.scala` -- see that file's doc for the general explanation (opaque-type transparency, scoped to the
  * WHOLE FILE a top-level opaque type is declared in, defeats companion-object-based extension resolution for any
  * `.someExtensionMethod` call made from that same file, silently for names that coincide with a real member of the
  * underlying representation type and with a hard compile error otherwise). Confirmed empirically for `Cube`
  * specifically, not just assumed to generalize from the `Simplex` case: moving this content back into
  * `Cubical.scala` reproduces "value encoded is not a member of Cube" / "value nondegenerateAxes is not a member of
  * Cube" at exactly the `x.encoded`/`c.nondegenerateAxes` call sites below.
  *
  ...
  * itself still relies on the transparency, but only in the file where it's defined, exactly like every other
  * extension here.
  */
