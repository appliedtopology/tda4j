...
/** A simplex of a finitely-generated simplicial set, in Eilenberg–Zilber normal form: `word` is the strictly decreasing
  ...
  * be non-degenerate. Dimension is `generator`'s own dimension plus `word.length`.
  *
  * Decreasing, not increasing: `s_i s_j = s_{j+1} s_i` for `i <= j` takes a non-decreasing adjacent pair to a
  * decreasing one, e.g. `s_0 s_0 = s_1 s_0` -- so the (unique) normal form for "apply `s_0` twice" is `[1, 0]`, never
  * `[0, 1]`.
  */
...
/** Composes a new outermost degeneracy `s_m` onto an already-normalized word, restoring the strictly-decreasing
  ...
  * untouched (already sorted); otherwise `m` must move past that index, incrementing it, and recurse.
  */
...
/** `d_i` on an arbitrary element `s_w1 ... s_wk (y)`, given only the primitive face data on generators (`faces(y)(i)` =
  * `d_i(y)`, for `y` non-degenerate). Bottoms out on `word = Nil`; otherwise pushes `d_i` past the outermost degeneracy
  * `s_w1` via the simplicial identities:
  ...
  *
  * `faces(y)` must return, for `y` of dimension `n`, exactly `n+1` already-normalized elements of dimension `n-1`
  * (empty for `n = 0`) -- see `FiniteSimplicialSet.validate()` for a runtime check of this contract.
  */
