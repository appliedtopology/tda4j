...
case class CofacetIterator[VertexT: Ordering](
  ...
  override def next(): VertexT =
    if hasNext then
      if outputQueue.nonEmpty then outputQueue.removeHead()
      else
        fillQueue()
        if outputQueue.nonEmpty then outputQueue.removeHead()
  ...
  // and returning the moment we find something in the intersection
  {
    ...
    while outerflag && (vi < vertices.size) do
      val neighbors = sparseMetricSpace.neighborhoods(vertices(vi))
      var wi: Int = 0
      var innerflag: Boolean = true
      while innerflag && (wi < neighbors.size) do
        if !simplex.contains(neighbors(wi)._1) then
          if neighbors(wi)._2 <= alpha then vertexL1.addOne(neighbors(wi)._1)
          else
            innerflag = false
            vPointers(vi) = wi
        wi += 1
      if innerflag then // we bailed out because we walked through all neighbors of vi
        vPointers(vi) = wi
      vi += 1
  }
  ...
  while !foundApparentFlag && vertexL1.nonEmpty do
    val w = vertexL1.dequeue()
    val seen = vertexCache.getOrElse(w, 0) + 1
    if seen == vertices.length then
  ...
  def process(): Unit =
    if !finishedInit then
      finishInit()
      finishedInit = true
    else if canProcess then
      Try {
        nearestNeighbor
      } match
        case Success((v, (w, d))) =>
          if !simplex.contains(w) then
            vertexCache(w) = vertexCache.getOrElse(w, 0) + 1
            if vertexCache(w) == vertices.size then
