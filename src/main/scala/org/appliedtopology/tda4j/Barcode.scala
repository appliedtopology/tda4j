...
given [FiltrationT](using ord: Ordering[FiltrationT]) : Ordering[BarcodeEndpoint[FiltrationT]] with
    def compare(x: BarcodeEndpoint[FiltrationT], y: BarcodeEndpoint[FiltrationT]) = x match {
      case NegativeInfinity[FiltrationT]() => -1
      case PositiveInfinity[FiltrationT]() => +1
      case ClosedEndpoint(xvalue) => y match {
        case NegativeInfinity[FiltrationT]() => +1
        case PositiveInfinity[FiltrationT]() => -1
        case ClosedEndpoint(yvalue) => ord.compare(xvalue, yvalue)
        case OpenEndpoint(yvalue) => if(ord.compare(xvalue, yvalue) == 0) -1 else ord.compare(xvalue, yvalue)
      }
      case OpenEndpoint(xvalue) => y match {
        case NegativeInfinity[FiltrationT]() => +1
        case PositiveInfinity[FiltrationT]() => -1
        case ClosedEndpoint(yvalue) => if(ord.compare(xvalue, yvalue) == 0) +1 else ord.compare(xvalue, yvalue)
...
case class PersistenceBar[FiltrationT : Ordering, AnnotationT](
  ...
  override def toString: String = {
    ...
    val annotationString : String = (for
...
object PersistenceBar {
  ...
  def apply[FiltrationT : Ordering](dim: Int) =
    new PersistenceBar[FiltrationT,Nothing](dim, NegativeInfinity[FiltrationT](), PositiveInfinity[FiltrationT]())
  def apply[FiltrationT : Ordering](dim: Int, lower: FiltrationT) =
    new PersistenceBar[FiltrationT,Nothing](dim, ClosedEndpoint(lower), PositiveInfinity[FiltrationT]())
  def apply[FiltrationT : Ordering](dim: Int, lower: FiltrationT, upper: FiltrationT) =
    new PersistenceBar[FiltrationT,Nothing](dim, ClosedEndpoint(lower), OpenEndpoint(upper))
}
...
class BarcodeContext[FiltrationT : Ordering]() {
   ...
   *
   */
  class BarAssembly(val lower: BarcodeEndpoint[FiltrationT], val upper: BarcodeEndpoint[FiltrationT])
  extension (lower: FiltrationT) {
    infix def bc(upper: FiltrationT) = new BarAssembly(ClosedEndpoint(lower), OpenEndpoint(upper))
    infix def clcl(upper: FiltrationT) = new BarAssembly(ClosedEndpoint(lower), ClosedEndpoint(upper))
    infix def clop(upper: FiltrationT) = new BarAssembly(ClosedEndpoint(lower), OpenEndpoint(upper))
    infix def opcl(upper: FiltrationT) = new BarAssembly(OpenEndpoint(lower), ClosedEndpoint(upper))
    infix def opop(upper: FiltrationT) = new BarAssembly(OpenEndpoint(lower), OpenEndpoint(upper))
  }
  def clinf(lower: FiltrationT): BarAssembly = new BarAssembly(ClosedEndpoint(lower), PositiveInfinity[FiltrationT]())
  def opinf(lower: FiltrationT): BarAssembly = new BarAssembly(OpenEndpoint(lower), PositiveInfinity[FiltrationT]())
  def infcl(upper: FiltrationT) = new BarAssembly(NegativeInfinity[FiltrationT](), ClosedEndpoint(upper))
  def infop(upper: FiltrationT) = new BarAssembly(NegativeInfinity[FiltrationT](), OpenEndpoint(upper))
  def dim(d: Int)(ba: BarAssembly) = new PersistenceBar[FiltrationT,Nothing](d, ba.lower, ba.upper)
}
...
class Barcode[FiltrationT : Ordering : Numeric, AnnotationT] {
  def isMap(
    ...
    source(c).lower >= target(r).lower && // target must be born when source is born
    source(c).upper >= target(r).upper && // source must still live when target dies
    source(c).lower <= target(r).upper && // source must be born before target dies
    source(c).upper >= target(r).lower    // target must be born before source dies
  )).forall { b => b }
  ...
  def image(
    ...
    yield {
      imageMatrix.setEntry(birth, death, matrixT.getEntry(birthOrder(birth), deathOrder(death)))
    }
    reduceMatrix(imageMatrix)
    (for
      (birth,death) <- pivotsOf(imageMatrix)
    yield {
      PersistenceBar[FiltrationT,AnnotationT](
        source(birth).dim,
        births(birth),
        deaths(death),
  ...
  def kernel(
              ...
              target: List[PersistenceBar[FiltrationT, AnnotationT]],
              matrix: RealMatrix
            )(using ord: Ordering[FiltrationT]): List[PersistenceBar[FiltrationT, AnnotationT]] = {
    ...
    val cokernelIntervals = cokernel(dualSource, dualTarget, matrix.transpose())(using ord=ord.reverse)
  ...
  def cokernel(
    ...
    yield {
      cokernelMatrix.setEntry(t,s,matrix.getEntry(birthOrder(t),deathOrder(s)))
    }
    (0 until target.size).foreach { t => cokernelMatrix.setEntry(birthOrder(t), deathOrder(source.size+t), 1.0) }
  ...
  def reduceMatrix(matrix: RealMatrix): RealMatrix = {
    ...
    yield {
      matrix.setColumnMatrix(nextcol,
        matrix.getColumnMatrix(nextcol).subtract(matrix.getColumnMatrix(col).scalarMultiply(matrix.getEntry(pivot,nextcol)/matrix.getEntry(pivot,col))))
    }
    matrix
  }
