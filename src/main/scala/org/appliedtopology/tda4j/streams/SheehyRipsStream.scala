...
/** The linear-size approximate/sparse Vietoris-Rips filtration of Cavanna, Jahanseir & Sheehy, "A Geometric
  * Perspective on Sparse Filtrations" (arXiv:1506.03797, CCCG 2015) -- checked directly against the paper's own
  ...
  * approximation (Theorem 5) to the plain Vietoris-Rips barcode, built from a complex whose SIZE is linear in `n`
  * (CJS 2015 Lemma 6/7/Theorem 9/10) for point sets of bounded doubling dimension -- dramatically fewer simplices to
  * reduce than plain VR at the same scale range, in exchange for a controlled, quantified loss of precision.
  *
  * '''What this class does NOT do''': the paper's own `O(n log n)` algorithm (Section 5, Algorithms 1-4) builds the
  * edges of the sparse filtration directly from the greedy permutation's neighbor structure, touching only
  * `kappa^O(d)` candidates per point. This class instead computes every pairwise `edgeBirth` directly (`O(n^2)`,
  * like plain `VietorisRips`/`RipserCofaceSimplexStream`'s own default candidate enumeration) and lets
  * `RipserCofaceSimplexStream`'s ordinary combinatorial coface generation do the rest. The payoff here is a SMALLER
  * complex to reduce, not a faster one to build -- see the class doc's own honest framing on this point in every
  * other "not the fastest construction" case in this codebase (Cech, Witness, alpha).
  *
  * ==The construction==
  *
  ...
  *
  *   - each point `p`'s ball radius at scale `alpha` is `r_p(alpha) := min(alpha, lambda_p*(1+epsilon)/epsilon)`
  *     (CJS 2015 Section 3) -- grows with `alpha` until it saturates;
  ...
  * published algorithm, not an artifact of skipping its neighbor-search prefilter: `epsilon=1, lambda_p=1,
  * lambda_q=10, d=10` (raw formula gives `8`, but `p` vanishes at `4`); and, checked directly against the paper's
  * OWN Lemma 6/7 neighbor bound (`kappa = (epsilon^2+3*epsilon+2)/epsilon`, `d(p_i,p_j) <= kappa*2^ceil(lg lambda_i)`
  * puts `p_j` in `p_i`'s own candidate neighbor list), `epsilon=1, lambda_p=1.1, lambda_q=10, d=10` -- this pair
  * passes the paper's own restricted neighbor search AND its Algorithm 3 (returning `7.8`), while `p` vanishes at
  * `4.4`. Whether the paper's full `O(n log n)` pipeline compensates for this some other way was not checked; only
  * the gap itself was verified. `edgeBirth` here applies the `vanish` clamp Section 5.3 describes to every edge, not
  * just higher simplices.
  *
  * '''Units''': every OTHER stream in this codebase records filtration values in "diameter" units (an edge's own
  * value is the raw ambient `distance`, not half of it) -- but CJS 2015's own `alpha` is a RADIUS parameter (`R_alpha
  * := {J : max d(p,q) <= 2*alpha}`; their own Algorithm 3 literally returns `d/2` in its first branch). Every output
  * this class reports -- `edgeBirth`'s return value and `vanish` -- is therefore the paper's own value DOUBLED, never
  * `lambda` itself (an ordinary ambient distance already in diameter-comparable units, entering the formulas
  * unchanged). After doubling, the first (unsparsified) branch collapses to exactly `d`, matching plain VR's own edge
  * value -- a useful internal sanity check, exercised directly by `SheehyRipsStreamSpec`'s "reduces to plain VR"
  * fixture.
  *
  * '''`maxFiltrationValue`''' is always clamped to `maxFiniteFiltrationValue` -- the largest FINITE simplex birth
  * this construction can ever produce (an honest, data-dependent bound: any finite simplex's value is a max over its
  * own edges, so the largest finite EDGE birth bounds every finite simplex) -- REGARDLESS of what the caller passes
  * (`None` resolves to exactly that bound; an explicit `Some(x)` is `min(x, maxFiniteFiltrationValue)`). This is
  * deliberately NOT `metricSpace.minimumEnclosingRadius` (this is not a cone construction -- an edge to the anchor
  * point can be unboundedly large, so no point has a finite max distance to every other point), and the clamp is
  * unconditional rather than only a `None`-default specifically so that an explicit `Some(Double.PositiveInfinity)`
  * -- how every other stream in this codebase spells "untruncated" -- stays safe here too:
  * `keptByThresholdAndCriterion`'s own `<=` comparison is plain IEEE-754 `Double` comparison, under which
  * `Double.PositiveInfinity <= Double.PositiveInfinity` is `true`, so an UNCLAMPED literal infinite threshold would
  * silently readmit every simplex this construction is supposed to exclude forever.
  *
...
object SheehyRipsSimplexStream:
  ...
  /** Builds the greedy permutation itself (via `LandmarkSelector.maxmin` run to full size, see that method's own doc)
    * before delegating to the primary constructor -- the convenience entry point most callers want. Validates
    * `epsilon` FIRST, before doing any of that `O(n^2)` work: the primary constructor's own `require` runs only
    * after its superclass's constructor arguments (including `maxFiniteFiltrationValue`) are already evaluated, so
    * relying on that alone would waste a full pairwise pass on an input that was always going to be rejected.
    */
  def apply(
  ...
  /** CJS 2015 Algorithm 3 (`EdgeBirthTime`), doubled into diameter units and extended with the `vanish` clamp
    * described in the class doc. `lambdaP`/`lambdaQ` are the two endpoints' own insertion radii (order doesn't
    * matter -- the smaller is found internally, exactly like the paper's own leading swap step), `d` their ambient
    * (undoubled) distance. Returns `Double.PositiveInfinity` for a pair that never appears.
    */
  ...
  /** `lambda*(1+epsilon)^2/epsilon`, in the SAME (undoubled, radius) units `edgeBirth`'s own internal `raw` is
    * computed in -- deliberately not doubled here, since `edgeBirth` compares against it before doubling its own
    * result. `vanishDoubled` below is the doubled, public version higher-dimensional simplices compare against.
    */
  ...
  /** Dimension 0 => 0.0 (every point is born at scale 0, exactly like plain VR -- `b_p(0) = ball(p,0) = {p}`, always
    ...
    * pairwise value already is one OR if that max exceeds the smallest `vanishDoubled` among the simplex's own
    * vertices (CJS 2015 Section 5.3's `SimplexBirthTime`, applied uniformly from dimension 1 up -- see the class
    * doc's note on why `edgeBirth` alone is not sufficient at dimension 1 either). Memoized: `Chain`'s reduction
    * consults `filtrationValue` on every pivot comparison, and this is an `O(k^2)` pairwise scan per simplex.
    */
        ...
        cache.getOrElseUpdate(
          ...
          if spx.dim <= 0 then 0.0
            ...
            val edgeBirths =
              ...
              yield edgeBirth(lambda(vertices(i)), lambda(vertices(j)), ambientMetricSpace.distance(vertices(i), vertices(j)), epsilon)
  ...
  /** The largest FINITE edge birth this construction produces on `ambientMetricSpace` -- a valid `maxFiltrationValue`
    * default because every finite simplex's own value is a `max` over its edges (see the class doc). Materializes
    * every pairwise `edgeBirth` once, `O(n^2)` like the rest of this reference implementation. `0.0` if every pair is
...
/** A full greedy permutation (farthest-point / "maxmin" sampling) of an ENTIRE finite metric space -- as opposed to
  * `streams.LandmarkSelection`, which picks a SUBSET. `order` is every ambient index in selection order (`order(0)`
  * is the seed point); `insertionRadius(p)` is `p`'s own `lambda_p = d(p, {points ordered before p})`, with
