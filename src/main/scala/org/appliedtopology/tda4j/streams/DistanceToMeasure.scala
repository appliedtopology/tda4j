...
/** The empirical distance-to-measure function (Chazal, Cohen-Steiner & Merigot, "Geometric inference for probability
  ...
  * measure and mass parameter `k` (equivalently `m0 = k/|X|`),
  *
  ...
  * counts as one of its own `k` neighbours, at distance 0) is likewise GUDHI's own convention -- confirmed both from
  * that class's own docstring ("k: number of neighbors (possibly including the point itself)") and by reproducing its
  ...
  * every `FiniteMetricSpace` in this codebase actually satisfies (`ExplicitMetricSpace` enforces nothing -- GUDHI's own
  * docs feed correlation-derived "distance" matrices through exactly this class). `JVPTree` is only safe over a genuine
  * metric.
  */
