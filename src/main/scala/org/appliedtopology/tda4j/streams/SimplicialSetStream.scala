...
class SimplicialSetStream[G](sset: FiniteSimplicialSet[G])(using G is OrderedCell) extends CellStream[G, Int]:
  ...
  export IntIsFilterable.{largest, smallest}
...
/** Builds a `FiniteSimplicialSet` from any stream of simplices: the faces of a genuinely-ordered simplex (a strictly
  * increasing vertex tuple) are always non-degenerate -- removing one entry from a strictly increasing sequence leaves
  * it strictly increasing -- so every generator's face data is a bare (non-degenerate) generator, `word = Nil`
