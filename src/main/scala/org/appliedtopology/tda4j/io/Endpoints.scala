...
/** Shared textual infinite-endpoint parsing/formatting (`inf`/`-inf`/`infinity`, any case), used by every
  * text-based persistence-diagram format in this package (`Csv`, `Gudhi`). `Dipha`'s binary persistence-diagram
  * format uses its own numeric essential-class convention instead (a negative dimension field) and doesn't need
  * this.
  */
private[io] object Endpoints:
  ...
  /** Builds a bar from raw birth/death tokens using this codebase's own half-open `[birth, death)` convention when
    * both are finite -- matching `PersistenceBar.apply(dim, lower, upper)` exactly, rather than wrapping both in
    * `ClosedEndpoint` -- so that a bar built the ordinary way (via that same factory) round-trips through a text
    * format unchanged: format a bar, re-parse it, and get back an `==` bar, not one whose `upper` silently flipped
    * from `OpenEndpoint` to `ClosedEndpoint`. `death == +infinity` matches `PersistenceBar.apply(dim, lower)`
    * (essential) the same way. An infinite `birth` (rare, but some formats allow it) falls back to the raw
    * `PersistenceBar` constructor, since neither companion factory covers that case.
    */
