...
/** Every simplex of a finitely-generated simplicial set is, by the same Eilenberg-Zilber normal-form theorem
  * `SSetElement` itself is built on, uniquely `(word, generator)` with `generator` non-degenerate of some
  * dimension `p <= n` and `word` a size-`(n - p)` subset of `{0, ..., n-1}` (sorted decreasing) -- so this
  * enumerates ALL of `X_n`, not just its generators, by pairing every generator at every dimension `p <= n`
  * with every such subset. Needed to build `product`, which has to consider every element of `X_n x Y_n`, not
  * just the non-degenerate ones, before filtering down to the pairs that are non-degenerate in the product.
  */
...
/** A pair `(a, b) in X_n x Y_n` is degenerate in the product iff some single `j` degenerates both sides at once
  * (`s_j` on `X x Y` is literally the diagonal `(s_j, s_j)`) -- and `a` is `s_j`-degenerate exactly when `j`
  * appears in `a.word` (the normal form's own meaning: `word` is precisely the set of indices `a` is degenerate
  * at, not just the outermost one -- e.g. `s_1 s_0(v)` is degenerate at both `s_1` directly and, via `s_0 s_0 =
  * s_1 s_0`, at `s_0` too, matching both entries of its word `[1, 0]`). So non-degeneracy of the pair is exactly
  * disjointness of the two words. Cross-checked against the definitional `sOp(j, dOp(j, a)) == a` test in
  * `SimplicialSetConstructionsSpec`.
  */
def isNonDegeneratePair[GX, GY](a: SSetElement[GX], b: SSetElement[GY]): Boolean =
  a.word.toSet.intersect(b.word.toSet).isEmpty

/** A non-degenerate `n`-simplex of `X x Y`, presented as the pair of `SSetElement`s (each possibly itself
  * degenerate over its own non-degenerate generator) whose word-sets are disjoint -- NOT an Eilenberg-Zilber
  * shuffle triple. The shuffle family indexes the classical chain MAP between `C_*(X) tensor C_*(Y)` and
  * `C_*(X x Y)`, not the product's own non-degenerate simplices: `(e_X, e_Y)`, both non-degenerate of dimension
  * 1, is a non-degenerate 1-simplex of `X x Y` that no `(p,q)`-shuffle with `p+q=1` could ever produce, since a
  * shuffle needs `p+q=n` but here `p=q=1=n`. See `.claude/WORKLOG-simplicial-set-constructions.md`.
  */
...
/** Product of two finite simplicial sets, `(X x Y)_n = X_n x Y_n` degreewise.
  *
  * Top dimension is provably `maxDim(x) + maxDim(y)`, not merely an analogy to CW-complex dimension: a
  * non-degenerate pair `(a,b)` at dimension `n` needs `a.word`/`b.word` to be disjoint subsets of an
  * `n`-element set, and each has size `n - dim(generator) >= n - maxDim` (since `dim(generator) <= maxDim`), so
  * disjointness forces `(n - maxDim(x)) + (n - maxDim(y)) <= n`, i.e. `n <= maxDim(x) + maxDim(y)` -- beyond
  * that bound, no pair can possibly be non-degenerate, permanently, not just at that one dimension.
  *
  * Face maps reuse `faceOf` on each side independently; the result can come back degenerate IN THE PRODUCT even
  * when the input pair wasn't (a nonempty common word entry can appear on both sides after taking a face), so
  * it's re-normalized by stripping the common degeneracy set `J` from both words and wrapping the stripped pair
  * as a new generator degenerate by exactly `J` -- valid for the same diagonal reason above: an `s_j` present in
  * both sides' own normal form at once is exactly the product's own `s_j` applied to what's left.
  */
...
/** Coproduct (disjoint union) of two finite simplicial sets, `(X + Y)_n = X_n + Y_n` -- no degeneracy
  * interaction between the two factors, unlike `product`: a generator's faces stay entirely within whichever
  * side it came from, so this is just tagging generators with `Left`/`Right` and delegating.
  */
