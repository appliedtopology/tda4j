...
/** A finished persistence computation, in a shape callable directly from MATLAB (or any other plain-Java caller) via
  ...
  * `int[][]` -- no Scala types, no generics, no `java.util.Map` -- since none of those marshal reliably across
  * MATLAB's Java bridge. See `Tda4j` for how this gets constructed and `WORKLOG-matlab-api.md` for the design
  * rationale, including what was deliberately left out of this first pass.
  *
  * Bars are indexed `0` until `size() - 1`, in no particular guaranteed order (the underlying engines don't sort
  * their output beyond grouping by dimension). An essential (never-dying) class reports `death(i) ==
  ...
  * array, with a parallel coefficient array) because MATLAB-side code that only wants "the simplices spanning this
  * bar" doesn't need to care which. Boundary-matrix export is not implemented yet -- flagged as a follow-up in
...
final class PersistenceResult private[matlab] (
  ...
  /** The whole barcode as one N-by-3 matrix: column 0 is dimension, column 1 is birth, column 2 is death (`+Inf` for
    * an essential class). This is the primary, MATLAB-idiomatic way to consume a result -- immediately plottable,
  ...
  /** The simplices making up bar `i`'s representative chain, each as its sorted array of vertex indices (0-based,
    ...
    * `UnsupportedOperationException` if this bar's engine doesn't record representative chains
    * (`engine="chunks"` currently never does) or if this specific bar has no recorded representative (can happen
    * for `engine="ripser"`, whose apparent-pairs shortcut skips writing one down for some bars).
    */
  def cycleVertices(i: Int): Array[Array[Int]] = cycleProvider(i)._1

  /** Coefficients parallel to `cycleVertices(i)`. Reported as `double` regardless of the underlying coefficient
    * field -- for a finite field `Z/pZ` this is the representative integer value cast to `double`, for the default
