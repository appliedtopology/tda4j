...
class HomologySpec extends Specification {
  """  HomologySpec """ should {
    """ be able to use Chain to compute a simple persistent homology barcode""" in {
      ...
      given chain_ctx: TDAContext[Int, Double] with {
        ...
        extension (chain: Chain[Simplex, Double]) {
          def reduceByBasis(basis: ListBuffer[Chain[Simplex, Double]]): Chain[Simplex, Double] = {
            var changed: Boolean = true
            var sigma = this
            while (changed) {
              changed = false
              for (basisElement <- basis) {
                if (sigma.leadingTerm == basisElement.leadingTerm) {
                  sigma = (basisElement.leadingCoefficient / sigma.leadingCoefficient) * sigma -
