...
/** Regression tests for the vertex-value/presence fix to `AlphaComplexDQPBuilder.compute()`: a vertex's own filtration
  ...
  * assuming this (the previous behaviour) is wrong whenever some Cech-neighbour's weight dominates x's own point, which
  * mild weights (as in `AlphaComplexDQPWeightedSpec`'s `[-0.3, 0.3]` range) never trigger but DTM weights do routinely.
  * See `.claude/WORKLOG-dtm-filtrations.md` for the full derivation; these two fixtures are its hand-worked oracles,
  * independent of any DTM machinery (plain `PowerDistance.euclidean` weights).
  */
