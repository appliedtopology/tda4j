...
/** Test-scope driver, NOT a scratch file (unlike this arc's earlier `PackedProfileDriver.scala`/ `ProfileDriver.scala`,
  ...
  * `/usr/bin/time -l` for peak RSS) -- see `.claude/WORKLOG-ripser-profiling.md`'s cursor-redesign session for why this
  * needs to be a real subprocess rather than another `-DpackedOnly`-style in-process sbt spec: a memory comparison
  * specifically needs one engine per OS process, which `RipserPaperBenchmarkSpec`'s shared-JVM, per-cell-timeout
  * harness doesn't give. `engine` is `"sortedset"` (`RipserCohomologyContext`) or `"packed"`
  * (`PackedRipserCohomologyContext`); `trials` (default 3) controls only the in-process wall-clock median -- a memory
  * measurement needs a fresh process per sample instead, driven externally by whatever wraps this.
  */
