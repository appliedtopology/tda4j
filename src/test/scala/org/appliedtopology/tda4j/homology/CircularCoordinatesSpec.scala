...
/** Tests `CircularCoordinates` (`.claude/WORKLOG-mainstream-feature-gap-analysis.md` item 2) against the textbook
  * example the construction is FOR: points sampled on a circle should recover an angle map affinely related to the
  * true geometric angle (up to the construction's own inherent ambiguities: a global phase/rotation, and an
  * orientation/sign flip, neither of which is meaningful to fix -- a cohomology class alone can't know "which way
  * is positive").
  */
class CircularCoordinatesSpec extends Specification:
  ...
  /** Independently finds the most-persistent H^1 bar's own (birth, death), via the SAME engine
    * CircularCoordinates.compute uses internally, but driven directly here rather than through that method --
    * this is how a real caller would find a sensible `r` too (there is no way to pick one without first knowing
    * the bar's own range), not a testing-only shortcut.
    */
  ...
  /** Circular mean resultant length of `xs` (values interpreted mod 1, i.e. angles `2*pi*x`): close to `1` means
    * `xs` is tightly clustered on the circle (low circular spread), close to `0` means spread out/uniform. The
    * standard way to test "is this sequence approximately constant mod 1" without the wraparound artifacts a
    * naive (non-circular) variance would introduce at the 0/1 boundary.
    */
  ...
  "essential representatives at K_r" should {
    "are genuine cocycles on both CellularCohomologyContext and PackedRipserCohomologyContext -- cross-checked " +
      ...
      val krStream = LimitedCofaceSimplexStream(EnumeratingCofaceSimplexStream(metricSpace, maxFiltrationValue = Some(r)), 2)
