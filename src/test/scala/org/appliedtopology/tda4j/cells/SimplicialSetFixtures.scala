...
object SimplicialSetFixtures:
  ...
  /** Minimal simplicial set model of `S^n`: one vertex `v`, one non-degenerate top `n`-cell `e`, whose `n+1` faces
    * are all the same maximally-degenerate `(n-1)`-simplex over `v` (for `n = 1`, that degenerate simplex is `v`
    * itself -- no degeneracy needed at all). `H_0 = H_n = F`, everything else `0`.
    */
  ...
  def minimalSphere(n: Int): FiniteSimplicialSet[SphereGenerator] =
    ...
    val byDim: IndexedSeq[Set[SphereGenerator]] =
      (0 to n).map(d => if d == 0 then Set(Vertex) else if d == n then Set(Top) else Set.empty[SphereGenerator])
        .toIndexedSeq
  ...
  /** Standard reduced-bar-construction model of `B(Z/2)`, truncated to its `topDim`-skeleton: one non-degenerate
    * generator `e_n` per dimension `0..topDim`. `d_0(e_n) = d_n(e_n) = e_{n-1}` (non-degenerate); for `0 < i <
    * n`, `d_i(e_n) = s_{i-1}(e_{n-2})` (Z/2's nontrivial generator squares to the identity). `topDim = 2` is RP²
    * (the sign-discriminating fixture: `H_1 = H_2 = F2` over F2, both `0` over F3); `topDim = 3` is RP³ (the
    * fixture that reaches `faceOf`'s `i > w1+1` branch, and pins an essential bar above dimension 0: `H_3 = F`
    * over every field, `H_1 = H_2 = 0` over F3).
    */
  ...
  def realProjectiveSpace(topDim: Int): FiniteSimplicialSet[ProjectiveGenerator] =
    ...
    val byDim: IndexedSeq[Set[ProjectiveGenerator]] = (0 to topDim).map(n => Set[ProjectiveGenerator](E(n))).toIndexedSeq
    def facesOf(g: ProjectiveGenerator): IndexedSeq[SSetElement[ProjectiveGenerator]] = g match
      ...
      case E(n) =>
        ...
        (0 to n).map(i => if i == 0 || i == n then outer else SSetElement[ProjectiveGenerator](List(i - 1), E(n - 2)))
          .toIndexedSeq
  ...
  /** Hatcher's minimal Δ-complex model of the torus (*Algebraic Topology*, Example 2.4): one vertex `v`, three
    * loop-edges `a, b, c` (`c` the diagonal), two triangles `u, l` with the SAME face assignment `d_0=b, d_1=c,
    * d_2=a`. Nothing here is degenerate -- its value is being a genuine Δ-complex (loops/repeated edges at a
    * single vertex) that `fromStream` can never produce from an actual `Simplex[VertexT]`. `H_0=F, H_1=F²,
    * H_2=F` for every coefficient field.
    */
  ...
  def torus: FiniteSimplicialSet[TorusGenerator] =
    ...
    val triangleFaces =
      IndexedSeq(SSetElement[TorusGenerator](Nil, B), SSetElement[TorusGenerator](Nil, C), SSetElement[TorusGenerator](Nil, A))
